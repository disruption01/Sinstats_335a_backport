...
-- Glyph Info (3.3.5a: GetGlyphSocketInfo returns enabled, glyphType, glyphSpell, icon)
local function checkGlyphs()
	...
	for i = 1, 6 do
		...
		if enabled and glyphSpellID then
			-- Original Wrath glyph effect spell IDs.
			if glyphSpellID == 56829 then
				Ns.glyphHuntersMark = 0.2
			elseif glyphSpellID == 63223 then
				Ns.glyphDivinePlea = 3
			elseif glyphSpellID == 58625 then
				Ns.glyphIcebound = 40
			elseif glyphSpellID == 55441 then
				Ns.glyphManaTide = true
			end
		end
	end
end
...
function Ns.StatsCompute()
	...
	for i = 1, 40 do
		local _, _, _, count, _, _, _, caster, _, _, spellId = UnitBuff("player",i, "HELPFUL")
		if not spellId then break end

		if spellId then Ns.buffCount = i end

		maxMana = UnitPowerMax("player", Ns.ManaPowerType)
	...
	elseif (Ns.classFilename == "HUNTER") then
		...
		--local playerMaxMana = UnitPowerMax("player", Ns.ManaPowerType)
...
function Ns.OnEventFunc(event, ...)
	...
	-----
	local _, subevent, _, sourceGUID, _, _, _, destGUID, _, _, _, spellId, spellName = Ns.GetCombatLogEventInfo(...)
	...
	if (Ns.manaTideCheck[spellId]) and (destGUID == playerGUID) and not Ns.tideTimer then
		local TideMana = UnitPowerMax("player", Ns.ManaPowerType)
	...
	if subevent == "SWING_DAMAGE" then
		if sourceGUID == playerGUID then
			amount, _, _, _, _, _, critical = select(12, Ns.GetCombatLogEventInfo(...))
			hitTimes = hitTimes + 1
	...
	elseif subevent == "SWING_MISSED" then
		if sourceGUID == playerGUID then
			missType = select(12, Ns.GetCombatLogEventInfo(...))
			hitTimes = hitTimes + 1
			Ns.meleeCrit = ((critTimes + spellcritTimes) / (hitTimes + spellhitTimes) * 100)
	...
	elseif subevent == "SPELL_DAMAGE" or subevent == "SPELL_HEAL" then
		if sourceGUID == playerGUID then
			_, _, _, _, _, _, healCrit, _, _, spellCritical = select(12, Ns.GetCombatLogEventInfo(...))
	...
	elseif subevent == "RANGE_DAMAGE" and sourceGUID == playerGUID then
		_, _, _, _, _, _, _, _, _, critical = select(12, Ns.GetCombatLogEventInfo(...))
	...
	if Ns.impMoonCheck then
		Ns.impMoon = 0
		Ns.impMoon = select(5, GetTalentInfo(1,19, Ns.canInspect, false))
	end
	if Ns.swiftRetCheck then
		Ns.impRet = 0
		Ns.impRet = select(5, GetTalentInfo(3,22, Ns.canInspect, false))
	end
	if Ns.faerieCheck then
		Ns.faerieFireImp = 0
		Ns.faerieFireImp = select(5, GetTalentInfo(1,15, Ns.canInspect, false))
	...
	if unit == "pet" and UnitCreatureFamily("pet") == "Imp" then
		for i = 1, 20 do
			local _, _, _, _, _, _, _, _, _, _, spellId = UnitAura("pet",i, "PLAYER")
			if not spellId then break end
			Ns.demonicEmp = 0
