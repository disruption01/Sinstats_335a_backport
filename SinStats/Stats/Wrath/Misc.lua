...
function Ns.FunctionList.Speed(HUD, data, options, ...)
	...
	-- 3.3.5a GetUnitSpeed only reliably returns the current speed. Wrath Classic
	-- additionally returns run/flight/swim speeds. Fall back to the current player
	-- speed when those extra return values are unavailable.
	local vehicleSpeed = GetUnitSpeed("vehicle") or 0
	local fullSpeed, runSpeed, flightSpeed, swimSpeed = GetUnitSpeed("player")
	fullSpeed = fullSpeed or 0
	runSpeed = runSpeed or fullSpeed
	flightSpeed = flightSpeed or runSpeed
	swimSpeed = swimSpeed or runSpeed
	local staticSpeed
	local returnText
	...
	staticSpeed = staticSpeed or fullSpeed or 0
...
function Ns.FunctionList.TargetSpeed(HUD, data, options, ...)
	...
	local targetSpeed = (GetUnitSpeed("target") or 0) / 7 * 100
	local speedColor = ""
