...
function Ns.FunctionList.GearScore(HUD, data, options, ...)
    ...
    if options.Score_Color then
        if r == 0.1 and g == 0.1 and b == 0.1 then
            ...
            scoreColor = string.format("|cff%02x%02x%02x", r * 255, g * 255, b * 255)
        end
    end
...
function Ns.FunctionList.ItemLevel(HUD, data, options, ...)
    ...
    if options.Dec_Misc_Ilvl == 0 then
        decimals = "%.0f"
    elseif options.Dec_Misc_Ilvl == 1 then
        decimals = "%.1f"
    elseif options.Dec_Misc_Ilvl == 2 then
        decimals = "%.2f"
    else
        decimals = "%.3f"
    end
...
function Ns.FunctionList.GlobalCD(HUD, data, options, ...)
    ...
    if options.Dec_Misc_Gcd == 0 then
        decimals = "%.0f"
    elseif options.Dec_Misc_Gcd == 1 then
        decimals = "%.1f"
    elseif options.Dec_Misc_Gcd == 2 then
        decimals = "%.2f"
    else
        decimals = "%.3f"
    end
...
function Ns.FunctionList.Speed(HUD, data, options, ...)
    ...
    local vehicleSpeed = GetUnitSpeed("vehicle") or 0
    local fullSpeed = GetUnitSpeed("player") or 0
    local flightSpeed = GetUnitSpeed("player") or 0
    local swimSpeed = GetUnitSpeed("player") or 0
    local staticSpeed = 0
    local returnText = ""

    if IsFlying() then
        staticSpeed = flightSpeed
    elseif IsSwimming() then
        staticSpeed = swimSpeed
    elseif vehicleSpeed > 0 then
        staticSpeed = vehicleSpeed
    elseif UnitOnTaxi("player") then
        staticSpeed = fullSpeed
    else
        staticSpeed = fullSpeed
    end
    ...
    speedColor = getColor(fullSpeed)
    staticColor = getColor(staticSpeed)

    if speedColor ~= "" then
        endColor = "|r"
    end

    if Ns.Band(EB, Live) then
        returnText = speedColor .. string.format("%d%%", ("%.0f"):format(fullSpeed))
    end

    if Ns.Band(EB, Static) then
        returnText = staticColor .. string.format("%d%%", ("%.0f"):format(staticSpeed))
    end

    if Ns.Band(EB, Both) then
        returnText = string.format("%s%d%%%s/%s%d%%", speedColor, fullSpeed, endColor, staticColor, staticSpeed)
    end
...
function Ns.FunctionList.Lag(HUD, data, options, ...)
    ...
    if Ns.Band(EB, World) then
        returnText = lagColor .. floor(lagWorld)
    end
    if Ns.Band(EB, Realm) then
...
function Ns.FunctionList.Durability(HUD, data, options, ...)
    ...
    if type(Durability) == "number" then
        if Durability > 50 then
            Text = string.format("|cff%2xff00",
                ((Durability > 50) and (255 - 2.55 * Durability) or (2.55 * Durability)), Durability) .. Text
        else
...
function Ns.FunctionList.Threat(HUD, data, options, ...)
    ...
    if threatpct <= 59 then
        threatColor = Ns.greenText
    elseif threatpct >= 60 and threatpct < 100 then
        threatColor = Ns.orangeText
    elseif threatpct == 100 then
        threatColor = Ns.redText
    end

    if Ns.Band(EB, Percentage) then
        returnText = threatColor .. ("%.0f%%"):format(threatpct)
    end
    if Ns.Band(EB, Rating) then
        returnText = threatColor .. ("%.0f"):format(threatvalue)
    end
    if Ns.Band(EB, Both) then
        returnText = threatColor .. ("%.0f%%"):format(threatpct) .. " |r(" .. ("%.0f"):format(threatvalue) .. ")"
    end
