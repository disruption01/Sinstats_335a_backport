...
local checkBoxHeight, radioSize, defaultLabelOsW, defaultLabelOsH, tabLabelOsW, MainMenuButtonHeight, topTabHeight = 30,
    14, 40, 10, 6, 21, 22
local topTabColor = {
    r = 0.298,
    g = 0.298,
...
local function AddLabel(self)
    -- Create the font string for the label
    self.Label = self:CreateFontString(nil, "OVERLAY")

    -- Define and apply functions from Mixins.functions.Labels
    local labelsMixin = Mixins.functions.Labels
    for k, v in pairs(labelsMixin) do
        self.Label[k] = v
    end

    -- Define and apply functions from Mixins.functions.HasFontString
    local hasFontStringMixin = Mixins.functions.HasFontString
    for k, v in pairs(hasFontStringMixin) do
        self[k] = v
    end
...
local function PlayOnTab()
    PlaySound(856, "Ambience") -- Replace with correct numeric ID for IG_QUEST_LOG_OPEN
end

local function PlayOnCheck()
    PlaySound(857, "Ambience") -- Replace with correct numeric ID for IG_MAINMENU_OPTION_CHECKBOX_ON
end
...
function Mixins.functions.CheckBoxFrame.Init(self, anchorframe, parent)
    self.MyInformation = parent.DisplayOrder[self:GetID()]
    self.Check.Settings = parent.MyInformation.svnotintable and parent.SvTable or
                              parent.SvTable[parent.MyInformation.stat]
...
function Mixins.functions.DropListFrame.Init(self, anchorframe, parent, displaylist)
    ...
    self.DropButton.Settings = parent.MyInformation.svnotintable and parent.SvTable or
                                   parent.SvTable[parent.MyInformation.stat]
...
function W.MainMenuButton(parent, width)
    ...
    f:GetNormalTexture():SetAlpha(0)
    f:SetHighlightTexture(Ns.TexturePath .. "HighlightMenu")
    f.Collapsed = true
    f:SetSize(width or 100, MainMenuButtonHeight or 20) -- Default values if width or height are nil

    -- Add scripts directly
    for k, v in pairs(Mixins.scripts.MainMenuButton or {}) do
        f:SetScript(k, v)
    end

    -- Debug prints
    print("Button created:", f:GetName())
    print("Button size:", f:GetWidth(), f:GetHeight())
...
function Mixins.functions.MainMenuButton.Expand(self)
    ...
    Ns.ConfigFrame.MainAnchor:SetLabel(format(Ns.WidgetDisplayFormat, Ns:GetSpellIcon(self.MyInformation),
        (L[self.MyInformation.stat])))
...
function W.ProfileConfig(parent) -- All the funcions to do Profiles
    ...
    function f.RunAction(self, action) -- The magic happens here.
        ...
        if action == "selected" then
            ...
            self.CurrentProfileText:SetText(format(CurrentProfileFormat, L["CurrentProfile"],
                SinStatsDB.profileKeys[Ns.ProfileKey]))
...
function Mixins.scripts.Slider.OnValueChanged(self, value)
    if self.Settings[self.Key] == value then
        return
    end
    self.Edit:SetValue(value)
...
function Mixins.scripts.TopTab.OnClick(self)
    ...
    if self.Properties.Checked then
        ...
        Ns.ConfigFrame.ChildAnchor.Description:SetText(format(Ns.WidgetDisplayFormat,
            Ns:GetSpellIcon(self.MyInformation), L[self.MyInformation.stat .. "Menu"])) -- .."Description" STATS TOP LINE TEXT
            ...
            if v.NewColumn and v.NewColumn.Settings then
                ...
                elseif action > 5 then
                    if counter > 1 and colAction == 1 then
                        ...
                        if (counter == widgetCount + 1) or (counter == (widgetCount * 2) + 1) or
                            (counter == (widgetCount * 3) + 1) then
                            if not v.NewColumn:GetChecked() then
                                v.NewColumn:Click()
                            end
                        else
